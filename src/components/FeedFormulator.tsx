...
import { PRESETS, generate500Ingredients, ANIMAL_NUTRITION_STANDARDS } from '../data/feedPresets';
...
export function FeedFormulator({ ingredients, onAddIngredientToLib, onDeleteIngredientToLib, onTriggerSectionReport, onAddInventoryItem }: FeedFormulatorProps) {
 ...
 let categoryColor = 'text-amber-500 bg-amber-900/20 border-amber-200';
 let isOptimal = false;
 ...
 categoryColor = 'text-green-600 bg-emerald-100 border-emerald-300';
 isOptimal = true;
 } else if (averageCp > 21) {
 categoryLabel = 'Elite Concentrated Booster (Adjust down with high-fiber fodders)';
 categoryColor = 'text-purple-600 bg-purple-900/20 border-purple-200';
 } else if (totalWeight > 0) {
 categoryLabel = 'Ration too lean in Crude Protein (<15% CP). Boost Cotton Cake / Soybeans.';
 categoryColor = 'text-rose-600 bg-rose-900/20 border-rose-200';
 }
 ...
 {/* Introduction banner */}
 <div className="bg-white shadow-sm p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
 <div className="flex items-center gap-4">
 <div className="p-3 bg-emerald-100 text-green-600 rounded-xl shrink-0">
 <Beaker size={24} />
 </div>
 <div>
 <h4 className="text-gray-900 font-semibold text-sm tracking-tight">Feed Formulation Lab</h4>
 ...
 type="button"
 className="flex items-center gap-1.5 px-3.5 py-2.5 bg-amber-500 hover:bg-amber-600 text-gray-500 rounded-xl font-bold text-xs  transition-all shadow-md cursor-pointer m-0 shrink-0 self-start sm:self-center border border-amber-600/10 font-bold"
 ...
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-100 shadow-sm space-y-6 lg:col-span-1">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-green-600 ">Ingredient Laboratory</h5>
 <p className="text-xs text-gray-900 font-medium mt-1 font-medium">Add materials to your compounding bank</p>
 </div>

 <form onSubmit={handleAddLabIngredient} className="space-y-4">
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Ingredient Name</label>
 <input
 type="text"
 required
 value={libName}
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Crude Protein (CP %)</label>
 <input
 type="number"
 required
 min="0"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Energy ME (MJ/kg)</label>
 <input
 type="number"
 required
 min="0"
 ...
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Cost / KG (Ksh)</label>
 <input
 type="number"
 min="0"
 step="0.5"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Category</label>
 <select
 value={libCategory}
 ...
 <label className="text-[10px] font-bold text-gray-900 font-medium tracking-tight block">
 ...
 className="text-xs border border-gray-200 rounded-xl py-2 px-3 pl-8 w-full outline-slate-350"
 />
 <span className="absolute left-2.5 top-3 text-[11px] text-gray-900 font-medium">🔍</span>
 </div>
 ...
 <p className="text-[10px] text-gray-900 font-medium italic text-center py-4">No matching materials found.</p>
 ) : (
 <>
 ...
 {ing.category && (
 <span className="text-[8px] bg-indigo-900/20 text-indigo-700 px-1 rounded-sm  font-semibold tracking-normal shrink-0">
 {ing.category}
 </span>
 )}
 </div>
 <p className="text-[10px] text-gray-900 font-medium mt-0.5 font-semibold">
 {ing.cost ? `Ksh ${ing.cost}/kg` : 'No price set'}
 </p>
 </div>
 <div className="text-right flex items-center gap-2">
 <div>
 <span className="font-mono font-bold bg-white shadow-sm text-green-600 border border-gray-200 px-1.5 py-0.5 rounded text-[10px] block">
 {ing.cp}% CP
 </span>
 <span className="font-mono text-gray-900 font-medium font-semibold block text-[9px] mt-0.5">
 {ing.me} MJ
 </span>
 </div>
 ...
 className="text-gray-500 hover:text-red-656 text-gray-900 font-medium hover:text-red-650 p-1 rounded transition-colors cursor-pointer m-0"
 ...
 <div className="bg-white shadow-sm p-8 rounded-3xl border border-gray-100 shadow-sm lg:col-span-2 space-y-6">
 {/* Mode Selector Tabs */}
 <div className="flex border-b border-gray-100 pb-2 gap-4">
 <button
 type="button"
 onClick={() => setFormulationMode('sandbox')}
 className={`pb-2.5 px-1 text-xs font-semibold tracking-tight transition-all border-b-2 cursor-pointer ${
 ...
 <div className="bg-indigo-900/20 p-5 border border-indigo-100/80 rounded-2xl space-y-4">
 <div className="flex justify-between items-center">
 <div className="flex items-center gap-1.5 text-indigo-950">
 <Sparkles size={14} className="text-indigo-700 font-semibold" />
 <span className="text-[11px] font-semibold  tracking-wide">Target Preset Library (By Classes)</span>
 </div>
 ...
 className={`text-[9px] font-semibold px-2.5 py-1 rounded-sm tracking-tight ${
 activePresetTab === tab 
 ...
 className="bg-white shadow-sm border border-indigo-200 hover:bg-indigo-900/20 text-indigo-900 font-semibold text-[10px] px-3 py-1.5 rounded-lg tracking-tight cursor-pointer m-0 transition-all shadow-xs"
 ...
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-green-600 ">Recipe Compounding Board</h5>
 <p className="text-xs text-gray-900 font-medium mt-1 font-medium">Add materials and specify relative weights (kilograms) in formula</p>
 </div>
 <div className="text-[9px] text-gray-900 font-medium font-bold  shrink-0">
 ...
 <span className="text-[9px] text-gray-900 font-medium font-bold ">
 ...
 className="text-xs bg-white shadow-sm border border-gray-200 rounded-xl p-2.5 w-full font-bold text-gray-900 font-semibold outline-none cursor-pointer"
 defaultValue=""
 >
 ...
 className="text-xs bg-white shadow-sm border border-gray-200 rounded-xl p-2.5 w-full font-semibold text-gray-900 placeholder-slate-400 outline-none focus:ring-1 focus:ring-emerald-500"
 />
 {recipeSearchQuery && (
 <button
 type="button"
 ...
 className="absolute right-3 top-3 text-gray-900 font-medium hover:text-gray-900 font-medium text-xs font-bold"
 >
 ✕
 </button>
 )}
 ...
 <div className="absolute left-0 right-0 mt-1 bg-white shadow-sm border border-gray-200 rounded-xl shadow-lg z-30 max-h-48 overflow-y-auto divide-y divide-slate-100">
 ...
 {ing.category && (
 <span className="text-[8px] bg-indigo-900/20 text-indigo-700 px-1 rounded  font-semibold tracking-normal">
 {ing.category}
 </span>
 )}
 </div>
 <div className="text-right shrink-0 flex items-center gap-1.5">
 <span className="font-mono font-bold bg-emerald-50 text-green-600 px-1.5 py-0.5 rounded text-[10px]">
 {ing.cp}% CP
 </span>
 <span className="text-[9px] text-gray-900 font-medium font-semibold mt-0.5 font-mono">
 {ing.me} MJ
 </span>
 </div>
 </button>
 ...
 <div className="absolute left-0 right-0 mt-1 bg-white shadow-sm border border-gray-200 rounded-xl p-3 shadow-lg z-30 text-center text-[10px] text-gray-900 font-medium italic">
 ...
 <span className="text-[10px] text-green-600 font-semibold font-mono bg-emerald-50 px-1.5 py-0.5 rounded">
 {rawIng?.cp ?? 0}% CP
 </span>
 <span className="text-[10px] text-sky-800 font-semibold font-mono bg-sky-900/20 px-1.5 py-0.5 rounded">
 {rawIng?.me ?? 0} MJ/kg
 </span>
 {rawIng?.cost && (
 <span className="text-[10px] text-gray-900 font-medium font-bold">
 Ksh {rawIng.cost}/kg
 </span>
 )}
 </div>
 ...
 placeholder="KG"
 className="text-xs border border-gray-200 rounded-xl p-2.5 w-full bg-white shadow-sm text-right font-mono font-semibold text-gray-900 outline-none focus:ring-1 focus:ring-emerald-500"
 />
 <span className="text-[10px] font-semibold text-gray-900 font-medium  font-mono">KG</span>
 </div>
 ...
 className="text-gray-900 font-medium hover:text-red-600 hover:bg-red-55 px-2.5 py-2.5 rounded-xl transition-all cursor-pointer m-0 border border-transparent hover:border-red-100"
 ...
 <div className="p-4 bg-white text-gray-900 rounded-2xl text-center shadow-md">
 <span className="text-[10px] text-green-600/90 font-semibold tracking-normal  block">Total Weight</span>
 <h3 className="text-3xl font-semibold font-mono mt-1 text-gray-900">{totalWeight.toFixed(1)} <span className="text-xs">KG</span></h3>
 </div>

 <div className="p-4 bg-white text-gray-900 rounded-2xl text-center shadow-md border-l border-emerald-900">
 <span className="text-[10px] text-green-600/90 font-semibold tracking-normal  block">Crude Protein</span>
 <h3 className="text-3xl font-semibold font-mono mt-1 text-yellow-500">{averageCp.toFixed(1)}<span className="text-xs">%</span></h3>
 </div>

 <div className="p-4 bg-white text-gray-900 rounded-2xl text-center shadow-md border-l border-emerald-900">
 <span className="text-[10px] text-green-600/90 font-semibold tracking-normal  block">Energy (ME)</span>
 <h3 className="text-3xl font-semibold font-mono mt-1 text-sky-400">{averageMe.toFixed(1)} <span className="text-[10px] font-normal">MJ/kg</span></h3>
 </div>

 <div className="p-4 bg-white text-gray-900 rounded-2xl text-center shadow-md border-l border-emerald-900">
 <span className="text-[10px] text-green-600/90 font-semibold tracking-normal  block">Est. Cost</span>
 <h3 className="text-xl font-semibold text-gray-900 font-mono mt-2 text-green-600">
 Ksh {totalCost.toLocaleString()}
 <span className="text-[10px] text-green-600 block font-normal">Ksh {averageCostPerKg.toFixed(1)}/kg</span>
 </h3>
 </div>
 </div>
 ...
 {isOptimal ? (
 <BadgeCheck size={20} className="shrink-0 text-green-600" />
 ) : (
 <ShieldAlert size={20} className="shrink-0" />
 )}
 <div>
 <span className="font-semibold  tracking-wide block">Formulation Safety Diagnostic:</span>
 <p className="mt-0.5 font-semibold text-gray-900 font-semibold">{categoryLabel}</p>
 </div>
 </div>
 ...
 <Sparkles size={20} className="shrink-0 text-green-600 animate-pulse" />
 <div className="text-left">
 <span className="font-semibold  tracking-wide block text-[#0d3621]">✨ Cross-Module Wagon Sync Active:</span>
 ...
 className="w-full py-3 bg-white hover:bg-emerald-900 text-green-600 border border-emerald-500/30 text-xs font-semibold tracking-tight rounded-xl transition-all cursor-pointer m-0 text-center shadow flex items-center justify-center gap-2"
 >
 ...
 {/* LCF UI Header */}
 <div className="bg-white shadow-sm text-gray-900 font-bold p-5 rounded-2xl border border-gray-200 space-y-2">
 <div className="flex items-center gap-2">
 <span className="text-[9px] bg-yellow-400 text-gray-500 px-2.5 py-0.5 rounded font-semibold tracking-tight">LINEAR OPTIMAL MATRIX SOLVER</span>
 <span className="text-[9px] text-gray-900 font-medium font-semibold ">Least-Cost Formulations (LCF)</span>
 </div>
 <h4 className="text-sm font-semibold text-gray-900 tracking-tight">Minimize formulation costs mathematically</h4>
 <p className="text-xs text-gray-900 font-medium leading-normal font-medium">
 ...
 <h4 className="text-xs font-semibold text-gray-900 font-semibold tracking-tight">Formulation Ratios Controls</h4>
 <p className="text-[10px] text-gray-900 font-medium mt-0.5">Select your basal and protein concentrates</p>
 </div>
 ...
 <div>
 <label className="text-[10px] font-semibold  text-gray-900 font-medium block mb-1">
 🎯 Target CP %: <span className="text-green-600 font-semibold font-mono">{pearsonsTargetCp}%</span>
 </label>
 <input
 type="number"
 min="5"
 ...
 {/* SVG Square Diagram */}
 <div className="flex items-center justify-center bg-white shadow-sm rounded-2xl p-4 border border-gray-200">
 ...
 <div className="bg-[#0f172a] text-gray-900 font-bold p-6 rounded-3xl space-y-5 border border-gray-200">
 <div className="flex justify-between items-center border-b border-gray-100 pb-3">
 <div>
 <span className="text-[9px] font-semibold text-green-600 tracking-tight block">Pearson's Square Formulation Result</span>
 <h4 className="text-sm font-semibold text-gray-900 mt-0.5 ">Exact 2-Ingredient Balancing Complete</h4>
 </div>
 <span className="text-[10px] font-semibold  px-2.5 py-1 rounded bg-emerald-500 text-gray-500">
 ...
 <div className="p-4 bg-gray-50 rounded-2xl border border-white/5 space-y-1 text-left">
 <span className="text-[9px] text-green-600 block  font-bold truncate max-w-full">{pearsonsResult.aObj.name}</span>
 <h3 className="text-xl font-semibold text-gray-900 text-gray-900 font-mono">{pearsonsResult.pctA.toFixed(1)}%</h3>
 <p className="text-xs text-gray-900 font-medium font-semibold font-mono">
 {pearsonsResult.kgA.toFixed(1)} kg of formula
 </p>
 <p className="text-[10px] text-gray-900 font-medium font-semibold font-mono">
 ...
 <div className="p-4 bg-gray-50 rounded-2xl border border-white/5 space-y-1 text-left">
 <span className="text-[9px] text-sky-400 block  font-bold truncate max-w-full">{pearsonsResult.bObj.name}</span>
 <h3 className="text-xl font-semibold text-gray-900 text-gray-900 font-mono">{pearsonsResult.pctB.toFixed(1)}%</h3>
 <p className="text-xs text-gray-900 font-medium font-semibold font-mono">
 ...
 {/* Aggregates */}
 <div className="p-4 bg-gray-50 rounded-2xl border border-white/5 space-y-1 text-left">
 <span className="text-[9px] text-amber-400 block  font-bold">Combined Feed Metrics</span>
 <h3 className="text-sm font-semibold text-gray-900 font-mono">Total: Ksh {pearsonsResult.totalCost.toLocaleString(undefined, {maximumFractionDigits: 0})}</h3>
 <p className="text-xs text-gray-900 font-medium font-semibold font-mono">
 Unit Cost: Ksh {pearsonsResult.avgCostPerKg.toFixed(1)}/kg
 </p>
 <p className="text-[10px] text-sky-400 font-semibold font-mono  tracking-wide">
 ...
 className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs  rounded-xl transition-all shadow-md text-center cursor-pointer m-0 border-none"
 >
 ...
 <p className="text-[11px] text-gray-900 font-medium italic">
 ...
 <div className="space-y-2">
 <label className="text-[10px] font-semibold  text-gray-900 font-medium block">
 ⚡ Target Metabolizable Energy: <span className="font-mono text-green-600 font-semibold text-xs">{lcfTargetMe} MJ/kg</span>
 </label>
 <input
 type="range"
 min="7"
 ...
 <span className="text-[10.5px]  font-semibold text-gray-900 tracking-normal">Candidate Feedstuffs list & inclusion bounds</span>
 <span className="text-[9px] text-gray-900 font-medium font-bold ">Toggle / Buy Price (KSH)</span>
 </div>
 ...
 className="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-emerald-505 accent-emerald-800 mt-0.5 shrink-0 cursor-pointer"
 />
 <div>
 <div className="flex items-center gap-2">
 <span className="text-xs font-bold text-gray-900 block truncate leading-tight max-w-[140px]" title={cand.name}>{cand.name}</span>
 <button
 type="button"
 onClick={() => {
 ...
 className="text-gray-500 hover:text-red-500 hover:bg-red-900/20 p-1 rounded transition-colors cursor-pointer"
 title="Remove Candidate"
 >
 <Trash2 size={11} />
 </button>
 </div>
 <span className="text-[9px] text-gray-900 font-medium font-bold block mt-1 font-mono ">
 {cand.cp}% CP • {cand.me} MJ
 </span>
 </div>
 </div>
 <div className="text-right shrink-0">
 <span className="text-[9px] text-gray-900 font-medium font-semibold  block leading-none">Price/kg</span>
 <div className="flex items-center gap-1 mt-1 font-mono">
 <span className="text-[9px] text-gray-900 font-medium font-bold">Ksh</span>
 <input
 type="number"
 value={cand.cost}
 onChange={(e) => {
 ...
 {cand.enabled && (
 <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-dashed border-gray-100 text-[10px] font-semibold text-gray-900 font-medium">
 <div>
 <span className="text-gray-900 font-medium block  text-[8px] font-bold">Min inclusion %</span>
 <input
 type="number"
 min="0"
 max="100"
 ...
 <div>
 <span className="text-gray-900 font-medium block  text-[8px] font-bold">Max inclusion %</span>
 <input
 type="number"
 min="1"
 max="100"
 ...
 className="w-full bg-[#0d3621] hover:bg-[#0c2f1e] text-gray-900 font-semibold text-xs  py-4 rounded-2xl transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md"
 >
 ...
 {lcfResult && (
 <div className="bg-[#0f172a] text-gray-900 p-5 rounded-3xl space-y-4 border border-gray-200 animate-fadeIn text-left">
 <div className="flex justify-between items-center border-b border-gray-100 pb-2.5">
 <div>
 <span className="text-[9px] font-semibold text-amber-400 tracking-tight block">Optimization Engine Report</span>
 <h4 className="text-xs font-semibold text-gray-900 mt-0.5">MATRIX SOLUTION REQUISITES MET</h4>
 </div>
 <span className="text-[10px] font-semibold  px-2.5 py-1 rounded-lg bg-emerald-500 text-gray-500">
 Feasible & Cost-Optimized
 </span>
 </div>

 <div className="grid grid-cols-3 gap-2.5">
 <div className="p-3 bg-gray-50 rounded-xl border border-white/5 text-center">
 <span className="text-[8.5px] text-gray-900 font-medium block  font-bold">Optimal Cost/kg</span>
 <span className="text-lg font-mono font-semibold text-gray-900 mt-1 block">
 Ksh {lcfResult.cost.toFixed(1)}
 </span>
 </div>
 <div className="p-3 bg-gray-50 rounded-xl border border-white/5 text-center">
 <span className="text-[8.5px] text-gray-900 font-medium block  font-bold">Crude Protein</span>
 <span className="text-lg font-mono font-semibold text-amber-400 mt-1 block">
 {lcfResult.cp.toFixed(1)}%
 </span>
 </div>
 <div className="p-3 bg-gray-50 rounded-xl border border-white/5 text-center">
 <span className="text-[8.5px] text-gray-900 font-medium block  font-bold">Metabolizable Energy</span>
 ...
 <span className="text-[9px]  font-semibold text-gray-900 font-medium block">Calculated Formulation Ratios:</span>
 ...
 className="w-full py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs  rounded-xl transition-all shadow-md text-center"
 >
 ...
 <div className="bg-white shadow-sm text-gray-900 font-bold p-6 sm:p-8 rounded-3xl border border-gray-200 shadow-xl space-y-6 animate-fadeIn mt-6 text-left">
 <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 border-b border-gray-200 pb-5">
 <div>
 <div className="flex items-center gap-2">
 <span className="text-[9px] bg-emerald-500 text-gray-500 px-2.5 py-0.5 rounded font-semibold tracking-tight animate-pulse">OFFICIAL NRC/KALRO REFERENCE</span>
 <span className="text-[9px] text-gray-900 font-medium font-semibold ">Standard Dietary Targets</span>
 </div>
 <h4 className="text-lg font-semibold text-gray-900 tracking-tight mt-1.5 font-sans">Animal Nutritional Requirements Guidelines</h4>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">
 ...
 <span className={`text-[8px] font-semibold px-2 py-0.5 rounded tracking-tight leading-none ${
 ...
 <span className="text-[10px] text-gray-900 font-medium font-mono font-bold">{req.intake}</span>
 </div>
 <div>
 <h5 className="text-xs font-semibold text-gray-900 tracking-tight leading-snug">{req.stage}</h5>
 <p className="text-[11px] text-gray-900 font-medium leading-relaxed font-medium mt-1 min-h-[50px] line-clamp-3 hover:line-clamp-none transition-all duration-200">
 {req.desc}
 </p>
 </div>
 </div>

 <div className="space-y-3 pt-3 border-t border-gray-200 border-dashed mt-4">
 <div className="grid grid-cols-2 gap-2">
 <div className="p-2 bg-white shadow-sm border border-gray-200 rounded-xl text-center">
 <span className="text-[8px] text-gray-900 font-medium block  font-bold tracking-normal">Protein (CP)</span>
 <span className="text-xs font-mono font-semibold text-amber-400 mt-0.5 block">{req.cpRange}</span>
 </div>
 <div className="p-2 bg-white shadow-sm border border-gray-200 rounded-xl text-center">
 <span className="text-[8px] text-gray-900 font-medium block  font-bold tracking-normal">Energy (ME)</span>
 <span className="text-xs font-mono font-semibold text-sky-400 mt-0.5 block">{req.meRange}</span>
 </div>
 </div>

 <div className="space-y-1 p-2.5 bg-white shadow-sm border border-gray-200 rounded-xl">
 <span className="text-[8px]  font-semibold text-gray-900 font-medium block tracking-normal">Ideal Feedstocks:</span>
 <p className="text-[10px] text-gray-900 font-medium leading-normal font-semibold italic truncate" title={req.ingredients}>
 {req.ingredients}
 </p>
 </div>
 </div>
