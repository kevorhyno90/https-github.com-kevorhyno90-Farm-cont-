...
export function Financials({ 
 ...
 <div className="farm-shell-panel p-6 rounded-[1.6rem] border border-gray-200 hover:shadow-[0_14px_40px_rgba(15,23,42,0.10)] transition-all duration-500 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
 <div className="flex items-center gap-4 text-left">
 <div className="p-3 bg-emerald-100 text-green-600 rounded-xl shrink-0 shadow-sm">
 <BookOpen className="text-green-600" size={24} />
 </div>
 <div>
 <h4 className="text-gray-900 font-semibold text-sm tracking-tight">Estate Accounting Ledger & Analytics</h4>
 ...
 type="button"
 className="flex items-center gap-1.5 px-4 py-2 bg-amber-500 hover:bg-amber-600 active:scale-95 transition-all duration-300 ease-in-out text-gray-500 font-semibold text-xs  rounded-xl transition-all shadow-md cursor-pointer m-0 border border-amber-600/10 font-bold shrink-0"
 ...
 <div className="flex flex-wrap gap-2.5 p-1.5 bg-white shadow-sm /70 rounded-2xl shadow-sm max-w-2xl text-xs border border-gray-200">
 <button
 onClick={() => setSubTab('ledger')}
 className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold transition-all tracking-tight m-0 border-none cursor-pointer ${
 subTab === 'ledger' ? 'bg-white shadow-sm text-gray-900 shadow-[0_4px_12px_rgb(0,0,0,0.05)] font-semibold' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold transition-all tracking-tight m-0 border-none cursor-pointer ${
 subTab === 'analytics' ? 'bg-white shadow-sm text-gray-900 shadow-[0_4px_12px_rgb(0,0,0,0.05)] font-semibold' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold transition-all tracking-tight m-0 border-none cursor-pointer ${
 subTab === 'budgets' ? 'bg-white shadow-sm text-gray-900 shadow-[0_4px_12px_rgb(0,0,0,0.05)] font-semibold' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold transition-all tracking-tight m-0 border-none cursor-pointer ${
 subTab === 'breeding_roi' ? 'bg-white shadow-sm text-gray-900 shadow-[0_4px_12px_rgb(0,0,0,0.05)] font-semibold' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 className={`flex items-center gap-2 px-4 py-3 rounded-xl font-bold transition-all tracking-tight m-0 border-none cursor-pointer ${
 subTab === 'granular_analysis' ? 'bg-white shadow-sm text-gray-900 shadow-[0_4px_12px_rgb(0,0,0,0.05)] font-semibold' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 <div className="bg-white shadow-sm text-gray-900 p-8 rounded-3xl border-8 border-double border-emerald-700/65 shadow-xl flex flex-col md:flex-row justify-between items-stretch gap-6 text-left">
 <div className="space-y-1.5 flex-1 flex flex-col justify-center">
 <span className="text-[10px] font-semibold  text-green-600 tracking-normal flex items-center gap-1.5">
 <Coins size={12} className="animate-spin text-green-600" /> NYARONDE FARM ACTIVE BALANCES
 </span>
 <h1 className={`text-5xl font-semibold font-mono tracking-tight ${netPl >= 0 ? 'text-green-600' : 'text-rose-400'}`}>
 Ksh {netPl.toLocaleString()}
 </h1>
 <p className="text-xs text-gray-900 font-medium">Integrated audit accounting of all dynamic inputs/outgoes</p>
 </div>
 ...
 <h3 className="text-green-600 font-semibold font-mono text-xl mt-2">+{totalIncome.toLocaleString()}</h3>
 ...
 <h3 className="text-rose-400 font-semibold font-mono text-xl mt-2">-{totalExpense.toLocaleString()}</h3>
 <span className="text-[9px] text-gray-900 font-medium block pt-1 font-medium italic">Accrued cash drains</span>
 </div>
 </div>
 </div>
 ...
 <h5 className="text-[11px] font-semibold tracking-normal text-green-600  flex items-center gap-1.5">
 <TrendingUp size={14} className="text-green-600" /> Log Income Stream
 </h5>
 <p className="text-xs text-green-600 font-medium mt-0.5">Record revenue payouts, sales advances, or cash receipts</p>
 </div>

 <form onSubmit={handleIncomeSubmit} className="space-y-4 text-left">
 <div>
 <label className="text-[10px] font-semibold text-green-600 tracking-tight block mb-1">Amount (Ksh)</label>
 <input
 type="number"
 required
 min="1"
 ...
 className="text-xs border border-emerald-200 rounded-lg p-3 w-full font-bold bg-white shadow-sm focus:ring-emerald-700/10 focus:outline-none"
 />
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <div>
 <label className="text-[10px] font-semibold text-green-600 tracking-tight block mb-1">Source / Category</label>
 <select
 required
 value={incSrc}
 onChange={(e) => setIncSrc(e.target.value)}
 className="text-xs border border-emerald-200 rounded-lg p-3 w-full font-semibold bg-white shadow-sm cursor-pointer focus:outline-none"
 >
 ...
 <div>
 <label className="text-[10px] font-semibold text-green-600 tracking-tight block mb-1">Details / Bill Notes</label>
 <input
 type="text"
 required
 value={incDesc}
 ...
 className="text-xs border border-emerald-200 rounded-lg p-3 w-full font-medium bg-white shadow-sm focus:outline-none"
 />
 </div>
 <div>
 <label className="text-[10px] font-semibold text-green-600 tracking-tight block mb-1">Transaction Date</label>
 <input
 type="date"
 required
 value={incDate}
 onChange={(e) => setIncDate(e.target.value)}
 className="text-xs border border-emerald-200 rounded-lg p-3 w-full font-bold bg-white shadow-sm font-mono cursor-pointer focus:outline-none"
 />
 </div>
 </div>
 <button
 type="submit"
 className="w-full bg-white hover:bg-emerald-900 active:scale-95 transition-all duration-300 ease-in-out text-gray-900 font-semibold text-xs  p-3.5 rounded-xl transition-all shadow-md m-0 border-none cursor-pointer"
 >
 ...
 {/* Expense logger */}
 <div className="bg-rose-900/20 border border-rose-150 p-6 rounded-3xl shadow-sm space-y-4">
 <div className="text-left">
 <h5 className="text-[11px] font-semibold tracking-normal text-rose-950  flex items-center gap-1.5">
 ...
 <div>
 <label className="text-[10px] font-semibold text-rose-950 tracking-tight block mb-1">Amount (Ksh)</label>
 <input
 type="number"
 required
 min="1"
 ...
 placeholder="Amount in Ksh"
 className="text-xs border border-rose-200 rounded-lg p-3 w-full font-bold bg-white shadow-sm focus:ring-rose-700/10 focus:outline-none"
 />
 </div>
 <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
 <div>
 <label className="text-[10px] font-semibold text-rose-950 tracking-tight block mb-1">Expense Category</label>
 <select
 required
 value={expSrc}
 onChange={(e) => setExpSrc(e.target.value)}
 className="text-xs border border-rose-200 rounded-lg p-3 w-full font-semibold bg-white shadow-sm cursor-pointer focus:outline-none"
 >
 <option value="">Choose category...</option>
 ...
 <div>
 <label className="text-[10px] font-semibold text-rose-950 tracking-tight block mb-1">Transaction Details</label>
 <input
 type="text"
 required
 value={expDesc}
 ...
 className="text-xs border border-rose-200 rounded-lg p-3 w-full font-medium bg-white shadow-sm focus:outline-none"
 />
 </div>
 <div>
 <label className="text-[10px] font-semibold text-rose-950 tracking-tight block mb-1">Transaction Date</label>
 <input
 type="date"
 required
 value={expDate}
 onChange={(e) => setExpDate(e.target.value)}
 className="text-xs border border-rose-200 rounded-lg p-3 w-full font-bold bg-white shadow-sm font-mono cursor-pointer focus:outline-none"
 />
 </div>
 </div>
 <button
 type="submit"
 className="w-full bg-rose-950 hover:bg-rose-900 active:scale-95 transition-all duration-300 ease-in-out text-gray-900 font-semibold text-xs  p-3.5 rounded-xl transition-all shadow-md m-0 border-none cursor-pointer"
 >
 ...
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-500 space-y-4 text-left">
 <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b border-gray-200 pb-4">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-[#0b251a]  flex items-center gap-1.5">
 <Filter size={14} className="text-teal-700" /> Operational Auditing Ledger
 </h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Granular drilldown of all processed accounts</p>
 </div>
 ...
 <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-gray-900 font-medium">
 <Search size={14} />
 </span>
 <input
 type="text"
 ...
 type="button"
 className="flex items-center gap-1.5 px-3.5 py-2.5 bg-emerald-50 hover:bg-emerald-100 active:scale-95 transition-all duration-300 ease-in-out border border-emerald-200 text-green-600 font-bold text-xs  rounded-xl transition-all shadow-xs cursor-pointer m-0 shrink-0"
 ...
 onClick={() => onTriggerSectionReport('financials')}
 type="button"
 className="flex items-center gap-1.5 px-3.5 py-2.5 bg-amber-500 hover:bg-amber-600 active:scale-95 transition-all duration-300 ease-in-out text-gray-500 text-gray-500 font-bold text-xs  rounded-xl transition-all shadow-md cursor-pointer m-0 shrink-0 border border-amber-600/10 font-bold"
 title="Download Financial PDF Report"
 >
 <Download size={13} />
 Download PDF Report
 ...
 {datePreset === 'custom' && (
 <div className="bg-teal-900/20 p-4 rounded-2xl border border-teal-100/50 grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 ...
 className="text-xs border border-gray-200 rounded-xl p-2.5 bg-white shadow-sm w-full font-bold font-mono focus:outline-none"
 />
 </div>
 <div>
 ...
 <td className="p-3 font-medium text-gray-900 font-medium max-w-xs truncate" title={r.description}>
 {r.description}
 </td>
 <td className="p-3 text-right font-mono font-semibold text-sm">
 <span className={r.type === 'income' ? 'text-green-600 text-green-600' : 'text-rose-700'}>
 ...
 className="text-gray-900 font-medium hover:text-indigo-800 p-2 border border-transparent hover:border-gray-200 rounded-lg transition-colors cursor-pointer m-0 bg-transparent"
 title="Edit entry"
 >
 <Edit2 size={13} />
 </button>
 ...
 className="text-gray-900 font-medium hover:text-green-600 p-2 border border-transparent hover:border-gray-200 rounded-lg transition-colors cursor-pointer m-0 bg-transparent"
 ...
 className="text-gray-500 text-gray-900 font-medium hover:text-red-650 p-2 border border-transparent hover:border-red-100 rounded-lg transition-colors cursor-pointer m-0 bg-transparent"
 title="Void entry"
 >
 <Trash2 size={13} />
 </button>
 ...
 {/* Timeline AreaChart */}
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-500 lg:col-span-8 space-y-4 text-left">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-[#0c2619]  flex items-center gap-1.5">
 <Calendar size={14} className="text-teal-700 animate-pulse" /> Monthly Cash Flow Trends (P&L Timeline)
 </h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Visualize monthly balance fluctuations and operational profit margins</p>
 </div>
 ...
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-500 lg:col-span-4 flex flex-col justify-between text-left">
 <div className="space-y-4">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-[#0c2619] ">Revenue Sources split</h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Where farm profits derive</p>
 </div>

 {incomeCategoryData.length === 0 ? (
 <div className="h-[200px] flex items-center justify-center text-gray-900 font-medium text-xs italic">
 ...
 {/* Legends list */}
 <div className="pt-4 border-t border-gray-200 text-[10px] space-y-1.5 font-bold tracking-tight text-gray-500">
 ...
 <span className="truncate max-w-[120px] text-gray-900 font-medium">{item.name}</span>
 </div>
 <span className="font-mono text-green-600">{percent}% ({item.value.toLocaleString()} Ksh)</span>
 </div>
 );
 })}
 </div>
 ...
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-500 space-y-4">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-rose-950 ">Capital Expenditures Allocation (Ksh)</h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Distribution of debits across farm operations</p>
 </div>

 {expenseCategoryData.length === 0 ? (
 <div className="h-[220px] flex items-center justify-center text-gray-900 font-medium text-xs italic">
 ...
 <div className="bg-white shadow-sm text-gray-900 p-6 rounded-3xl border border-gray-200 shadow-md flex flex-col justify-between">
 <div className="space-y-4">
 <span className="text-[8px] bg-red-500 text-gray-900 px-2 py-0.5 rounded font-semibold tracking-tight block w-fit">
 Urgent Operating Margin Alert
 </span>
 <h4 className="text-lg font-semibold tracking-tight leading-snug">
 ...
 <div>
 <span className="text-[9px] text-gray-900 font-medium block  font-bold text-right">Standard Threshold limit</span>
 <span className="text-green-600 font-mono font-semibold text-right block mt-1">40% Max target</span>
 </div>
 </div>
 </div>

 <div className="pt-4 border-t border-gray-200 text-[10px] text-gray-900 font-medium font-semibold leading-relaxed">
 {totalIncome === 0 ? (
 ...
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-500 text-left space-y-4">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-[#0c2619]  flex items-center gap-1.5 font-sans">
 <BarChart2 size={14} className="text-teal-700" /> Agribusiness Division ROI Comparative Analysis
 </h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Comparative review of net profits and estimated ROI percentages across farm sectors</p>
 </div>
 ...
 {subTab === 'budgets' && (
 <div className="bg-white shadow-sm p-8 rounded-3xl border border-gray-200 shadow-sm space-y-6 text-left">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-[#092215]  flex items-center gap-1.5">
 <Target size={14} className="text-rose-800" /> Capital Allocation & Expense Budgeting
 </h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Set monthly maximum allocation thresholds, review actual expenditures, and isolate over-spending on-the-fly.</p>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
 {/* Visual Progress bar meters */}
 <div className="space-y-5">
 <span className="text-[10px] text-gray-900 font-medium font-semibold tracking-normal  block">
 ...
 const textColor = pct > 100 ? 'text-red-700 font-semibold' : (pct >= 70 ? 'text-amber-700' : 'text-gray-900 font-medium');

 return (
 <div key={index} className="space-y-1.5 font-sans">
 <div className="flex justify-between items-center text-xs font-bold">
 <span className="text-gray-500  text-[10px] tracking-wide">{category}</span>
 ...
 {pct > 100 && (
 <div className="flex items-center gap-1 text-[9px] text-red-650 font-bold tracking-tight animate-pulse pt-0.5">
 ...
 <div key={index}>
 <label className="text-[9px] text-gray-900 font-medium font-bold block mb-1  truncate" title={category}>
 {category} (Ksh)
 </label>
 <input
 type="number"
 ...
 className="text-xs font-mono font-bold bg-white shadow-sm border border-gray-200 rounded-xl p-2.5 w-full focus:ring-1 focus:ring-rose-800 focus:outline-none"
 />
 </div>
 ))}
 </div>

 <div className="p-3.5 bg-[#0e301d]/5 rounded-2xl border border-[#0e301d]/10 text-[10px] text-gray-900 font-medium  font-bold leading-normal flex items-start gap-2.5">
 <Sparkles size={16} className="text-green-600 shrink-0 mt-0.5 animate-pulse" />
 <p>
 ...
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-teal-900  flex items-center gap-1.5">
 <BrainCircuit size={14} className="text-teal-700 animate-pulse" /> Dynamic Breeding ROI & Milking Projections
 </h5>
 <p className="text-xs text-gray-900 font-medium font-medium mt-0.5">Smart forecasting algorithm cross-linking artificial insemination timelines with upcoming lactation peak revenues.</p>
 </div>

 <div className="bg-indigo-900/20 border border-indigo-100 px-3 py-1 rounded-full text-[10px] font-bold text-indigo-950  shrink-0 flex items-center gap-1">
 ...
 <span className="text-[9px] text-green-600 font-semibold  block tracking-normal">Estimated Revenue Peak</span>
 <h3 className="text-2xl font-semibold text-gray-900 font-mono text-green-600 mt-1">Ksh {breedingPredictionData.projectedIncome.toLocaleString()}</h3>
 <span className="text-[10px] text-gray-500 font-medium block pt-1">At Ksh {activeMarketMilkPrice}/L sale price</span>
 </div>
 </div>
 ...
 <div className="space-y-4">
 <span className="text-[10px] text-gray-500 font-semibold tracking-tight block">
 ...
 <thead>
 <tr className="border-b border-gray-200 text-[9.5px]  font-semibold text-gray-900 font-medium">
 ...
 <tbody className="divide-y divide-slate-50 font-medium text-gray-500">
 {breedingPredictionData.upcomingCalvings.map((c, i) => {
 const daysColor = c.daysLeft <= 30 ? 'text-red-700 font-semibold' : (c.daysLeft <= 60 ? 'text-amber-705 text-amber-700' : 'text-gray-900 font-medium');
 const peakDaily = c.breed.toLowerCase().includes('friesian') ? 28 : (c.breed.toLowerCase().includes('jersey') ? 19 : 24);
 ...
 <td className="py-3 font-mono text-gray-900 font-semibold">{c.dueDate}</td>
 <td className="py-3">
 <span className={`text-[11px] font-bold ${daysColor}`}>
 {c.daysLeft <= 0 ? '✓ Calved / Peak Lactation' : `In ${c.daysLeft} days`}
 </span>
 </td>
 <td className="py-3 text-right font-mono font-bold text-gray-900 font-semibold">~{peakDaily} L/day</td>
 <td className="py-3 text-right font-mono font-semibold text-green-600">Ksh {expectedSum.toLocaleString()}</td>
 </tr>
 );
 })}
 </tbody>
 ...
 <div className="bg-white shadow-sm border border-gray-200 rounded-2xl p-5 shadow-xs">
 <span className="text-[10px] text-gray-900 font-medium font-semibold tracking-tight block">
 {granularViewMode === 'cow' ? 'Total Dairy Revenue' : 'Total Crop Revenue'}
 </span>
 <h3 className="text-xl font-semibold text-gray-900 text-green-600 font-mono mt-1">
 Ksh {granularViewMode === 'cow' 
 ? granularPnlData.totalCowRevenue.toLocaleString() 
 : granularPnlData.totalBlockRevenue.toLocaleString()}
 </h3>
 <p className="text-[9px] text-gray-900 font-medium font-bold  tracking-wide mt-1">
 ...
 <h3 className="text-xl font-semibold text-gray-900 text-rose-800 font-mono mt-1">
 Ksh {granularViewMode === 'cow' 
 ...
 <div className="bg-teal-950 text-gray-900 rounded-2xl p-5 border border-teal-900 shadow-sm">
 <span className="text-[10px] text-teal-300 font-semibold tracking-tight block">
 Average Cost Efficiency
 </span>
 <h3 className="text-xl font-semibold text-gray-900 font-mono mt-1">
 {granularViewMode === 'cow' ? (
 <>Ksh {(granularPnlData.totalCowRevenue > 0 ? (granularPnlData.totalCowCost / (granularPnlData.totalCowRevenue / activeMarketMilkPrice)) : 0).toFixed(1)} <span className="text-[10px] text-teal-300 font-mono font-semibold">/ Liter</span></>
 ) : (
 <>Ksh {(fields.length > 0 ? (granularPnlData.totalBlockCost / fields.reduce((sum, f) => sum + f.acreage, 0)) : 0).toLocaleString(undefined, {maximumFractionDigits:0})} <span className="text-[10px] text-teal-300 font-mono font-semibold">/ Acre</span></>
 )}
 </h3>
 <p className="text-[9px] text-teal-200 font-bold  tracking-wide mt-1">
 ...
 granularViewMode === 'cow' ? 'bg-white shadow-sm text-gray-900 shadow-sm' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 className={`flex-1 py-3 text-xs tracking-tight font-semibold rounded-xl transition-all m-0 cursor-pointer border-none ${
 granularViewMode === 'block' ? 'bg-white shadow-sm text-gray-900 shadow-sm' : 'text-gray-900 font-medium hover:text-gray-900'
 }`}
 >
 ...
 <div className="lg:col-span-8 bg-white shadow-sm border border-gray-200 rounded-3xl p-6 shadow-xs space-y-4">
 <div className="flex justify-between items-center flex-wrap gap-2">
 <div>
 <h4 className="text-sm font-semibold text-gray-900 ">
 {granularViewMode === 'cow' ? 'Cattle Profitability Ledger' : 'Block & Crop Cost of Production Ledger'}
 </h4>
 <p className="text-[10.5px] text-gray-900 font-medium font-bold ">
 ...
 <tbody className="divide-y divide-slate-100 font-medium text-gray-500">
 {granularViewMode === 'cow' ? (
 ...
 <td className="py-3.5 font-mono text-gray-900 font-semibold">
 {c.status === 'Lactating' ? (
 <span className="font-bold text-gray-900">{c.milkLiters.toFixed(1)} L</span>
 ) : (
 <span className="text-gray-900 font-medium">—</span>
 )}
 {c.isEstimated && (
 <span className="block text-[8px] text-teal-655 font-semibold ">Estimated</span>
 )}
 </td>
 <td className="py-3.5 text-right font-mono text-rose-805">
 Ksh {c.totalCost.toLocaleString(undefined, {maximumFractionDigits:0})}
 <span className="block text-[8.5px] text-gray-900 font-medium font-bold tracking-tight">
 Feed: {c.feedCost.toLocaleString(undefined, {maximumFractionDigits:0})} | Vet: {c.vetCost}
 </span>
 </td>
 <td className="py-3.5 text-right font-mono text-green-600">
 Ksh {c.totalRevenue.toLocaleString(undefined, {maximumFractionDigits:0})}
 </td>
 <td className={`py-3.5 text-right font-mono font-semibold ${isProfit ? 'text-green-600' : 'text-rose-850'}`}>
 ...
 <span className="block text-[8px] text-gray-500 font-semibold ">per liter</span>
 </td>
 </tr>
 );
 })
 ...
 <span className="text-[8px] bg-indigo-55 text-indigo-700 font-semibold  px-1.5 py-0.5 rounded-sm mt-1 self-start">
 {b.status}
 </span>
 </td>
 <td className="py-3.5 font-mono text-gray-900 font-semibold">
 <span className="font-bold text-gray-900">{b.acreage} Acres</span>
 {b.isEstimated && (
 <span className="block text-[8px] text-teal-655 font-semibold ">Estimated</span>
 )}
 </td>
 ...
 <span className="block text-[8px] text-gray-500 font-semibold ">per acre</span>
 </td>
 </tr>
 );
 })
 ...
 {/* Chart Comparison */}
 <div className="bg-white shadow-sm border border-gray-200 rounded-3xl p-6 shadow-xs space-y-4">
 ...
 <span className="text-[10px] font-semibold text-green-600 tracking-tight">
 Granular P&L Advisor Recommendation
 </span>
 </div>
 <div className="text-xs text-green-600 space-y-2.5 font-semibold leading-relaxed">
 {granularViewMode === 'cow' ? (
 <>
 <p>
 ...
 <div className="bg-white shadow-sm rounded-3xl w-full max-w-md shadow-2xl p-6 border border-gray-200 space-y-4 animate-fadeIn text-left">
 <div className="flex justify-between items-center pb-2 border-b border-gray-200">
 <h3 className="text-sm font-semibold  text-gray-900">Edit Transaction Flow</h3>
 <button onClick={() => setEditingFinancial(null)} className="text-gray-900 font-medium hover:text-gray-900 font-medium font-bold m-0 cursor-pointer bg-transparent border-none">✕</button>
 </div>
 <div className="space-y-3">
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Flow Type</label>
 <select
 value={editingFinancial.type}
 onChange={(e) => setEditingFinancial({ ...editingFinancial, type: e.target.value as 'income' | 'expense' })}
 className="border border-gray-200 rounded-lg p-3 w-full text-xs font-bold focus:outline-none"
 >
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Amount (Ksh)</label>
 <input
 type="number"
 value={editingFinancial.amount}
 onChange={(e) => setEditingFinancial({ ...editingFinancial, amount: parseInt(e.target.value) || 0 })}
 className="border border-gray-200 rounded-lg p-3 w-full text-xs font-bold font-mono focus:outline-none"
 />
 </div>
 <div>
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-500  block mb-1">Source / Category</label>
 <input
 type="text"
 value={editingFinancial.category}
 onChange={(e) => setEditingFinancial({ ...editingFinancial, category: e.target.value })}
 className="border border-gray-200 rounded-lg p-3 w-full text-xs font-semibold focus:outline-none"
 />
 </div>
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Description</label>
 <textarea
 ...
 className="px-5 py-2.5 bg-indigo-950 text-gray-900 rounded-lg text-xs font-semibold  hover:bg-indigo-900 active:scale-95 transition-all duration-300 ease-in-out m-0 shadow cursor-pointer border-none"
 >
 Save Changes
 </button>
 </div>
 ...
 {selectedInvoiceTx && (
 <div className="fixed inset-0 bg-white shadow-sm flex items-center justify-center p-4 z-[999]">
 ...
 <span className="text-[10px] font-semibold tracking-normal text-green-600  font-mono">JR Farm Estate</span>
 <h4 className="text-lg font-semibold text-gray-900  mt-1">Official Sales Receipt</h4>
 <p className="text-[10px] text-gray-900 font-medium font-mono mt-1 font-bold">Transaction Ref: {selectedInvoiceTx.id}</p>
 </div>
 
 <div className="space-y-4 text-xs">
 <div className="flex justify-between border-b border-gray-200 pb-2">
 <span className="text-gray-900 font-medium font-bold ">Transaction Date</span>
 <span className="font-mono font-bold text-gray-900 font-semibold">{selectedInvoiceTx.date}</span>
 </div>
 <div className="flex justify-between border-b border-gray-200 pb-2">
 <span className="text-gray-900 font-medium font-bold ">Account Class</span>
 <span className="font-semibold text-gray-900 ">{selectedInvoiceTx.category}</span>
 </div>
 <div className="flex justify-between border-b border-gray-200 pb-2">
 <span className="text-gray-900 font-medium font-bold ">Flow Direction</span>
 <span className={`font-semibold  ${selectedInvoiceTx.type === 'income' ? 'text-green-600' : 'text-rose-705'}`}>{selectedInvoiceTx.type}</span>
 </div>
 ...
 <span className="text-2xl font-semibold text-gray-900 text-green-600 font-mono">Ksh {selectedInvoiceTx.amount.toLocaleString()}</span>
 </div>
 </div>
 
 <div className="text-center text-[9px] text-gray-900 font-medium font-bold tracking-tight pt-4 border-t border-dashed border-gray-200">
 ...
 className="w-1/2 py-2.5 bg-white hover:bg-emerald-900 active:scale-95 transition-all duration-300 ease-in-out text-gray-900 rounded-xl text-xs font-semibold tracking-tight cursor-pointer shadow flex items-center justify-center gap-2 border-none"
 >
 <Printer size={13} />
 Print PDF Receipt
 </button>
