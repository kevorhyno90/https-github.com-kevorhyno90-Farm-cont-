...
How can I assist you with livestock, crop health, or navigating this app today?`
 ...
 type="button"
 className="flex items-center gap-2.5 bg-gradient-to-r from-emerald-950 to-emerald-900 border border-emerald-800 text-white font-bold hover:text-white px-4.5 py-3.5 rounded-full shadow-2xl hover:scale-105 transition-all text-xs font-black uppercase tracking-wider relative group outline-hidden cursor-pointer"
 >
 ...
 type="button"
 className="p-1.5 text-white font-medium font-medium hover:text-white rounded-lg hover:bg-slate-900 /10 transition-all cursor-pointer"
 >
 <X size={16} />
 </button>
 </div>
 ...
 <div 
 ref={scrollRef}
 className="flex-1 overflow-y-auto p-4 space-y-3.5 bg-slate-900 scrollbar-thin scrollbar-thumb-slate-800"
 >
 {messages.map((m, idx) => {
 ...
 isModel 
 ? 'bg-slate-900 text-white font-bold border border-slate-800 rounded-tl-xs' 
 ...
 <span className="text-white font-bold leading-normal">{renderedLine}</span>
 </div>
 );
 }
 ...
 return (
 <p key={lIdx} className={`${lIdx > 0 ? 'mt-1' : ''} ${isHeading ? 'text-[11.5px] font-black text-emerald-400 uppercase tracking-wide mt-3 pb-0.5 border-b border-emerald-950/40' : 'text-white font-bold'}`}>
 {renderedLine}
 </p>
 );
 })}
 ...
 {isLoading && (
 <div className="flex items-center gap-2 text-[10px] text-white font-medium font-medium font-mono pl-6 py-2">
 ...
 <div className="px-4 py-2 bg-slate-900 border-t border-slate-800 flex gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none scroll-smooth">
 {suggestions.map((s, idx) => (
 <button
 key={idx}
 onClick={() => handleSendMessage(s)}
 type="button"
 className="bg-slate-800 hover:bg-slate-700/80 text-white font-medium hover:text-white border border-slate-750 px-2.5 py-1 rounded-full text-[9px] font-bold uppercase transition-all duration-150 cursor-pointer text-left shrink-0"
 >
 {s}
 </button>
 ))}
 ...
 className="flex-1 bg-slate-900 text-white font-bold placeholder:text-white font-medium font-medium text-xs px-3.5 py-2.5 rounded-xl border border-slate-800 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-hidden font-medium transition-all"
 />
 <button
 type="submit"
