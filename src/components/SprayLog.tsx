...
export function SprayLog({ sprayRecords, onAddSpray, onDeleteSpray, onEditSprayRecord, onTriggerSectionReport }: SprayLogProps) {
 ...
 <div>
 <h4 className="text-gray-900 font-semibold text-sm tracking-tight">GlobalGAP Spray Compliance Register</h4>
 ...
 {/* Spray Event Logger */}
 <div className="bg-white shadow-sm p-6 rounded-3xl border border-gray-100 shadow-sm space-y-5 lg:col-span-1 h-fit">
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-red-900 ">Document Spray Treatment</h5>
 <p className="text-xs text-gray-900 font-medium mt-1 font-medium">Log active block fungicide/pesticide sprays</p>
 </div>
 ...
 <p className="text-[10px] text-gray-900 font-medium leading-tight">Click to instant-populate standard GlobalGAP treatment guidelines:</p>
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Target Treatment Block</label>
 <input
 type="text"
 required
 value={block}
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Chemical/Brand Used</label>
 <input
 type="text"
 required
 value={chemical}
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Sprayed Execution Date</label>
 <input
 type="date"
 required
 value={sprayDate}
 onChange={(e) => handleSprayDateChange(e.target.value)}
 className="text-xs border border-gray-200 rounded-lg p-3 w-full font-bold font-mono cursor-pointer"
 />
 </div>

 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">PHI Interval (Days)</label>
 <input
 type="number"
 required
 min="0"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block mb-1">Target Pest / Disease</label>
 <input
 type="text"
 required
 value={target}
 ...
 <span className="text-[10px] font-semibold text-gray-900 font-medium tracking-tight block">📅 Next Treatment Schedule</span>
 <div className="grid grid-cols-2 gap-3">
 <div>
 <label className="text-[10px] font-semibold text-gray-500 tracking-tight block mb-1">Interval Days</label>
 <input
 type="number"
 min="1"
 max="365"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-500 tracking-tight block mb-1">Next Spray Date</label>
 <input
 type="date"
 value={nextSprayDate}
 ...
 <button
 type="submit"
 className="w-full bg-red-950 hover:bg-red-900 text-gray-900 font-semibold text-xs  p-3.5 rounded-xl transition-all shadow-md m-0"
 >
 ...
 <div>
 <h5 className="text-[11px] font-semibold tracking-normal text-gray-500 ">Active Quarantine Controls & Safe Dates</h5>
 <p className="text-xs text-gray-900 font-medium font-medium">Real-time Safety Checkpoints for Plucking Managers</p>
 </div>
 <div className="flex items-center gap-2">
 <button
 onClick={downloadSprayCSV}
 type="button"
 className="flex items-center gap-1.5 px-3 py-2 bg-rose-900/20 hover:bg-rose-100 border border-rose-200 text-rose-950 rounded-xl font-semibold text-[10px]  transition-all shadow-xs cursor-pointer m-0"
 ...
 type="button"
 className="flex items-center gap-1.5 px-3 py-2 bg-amber-500 hover:bg-amber-600 text-gray-500 rounded-xl font-semibold text-[10px]  transition-all shadow-md cursor-pointer m-0 border border-amber-600/10 font-bold"
 ...
 <p className="text-xs text-gray-900 font-medium italic py-6 text-center">No spray intervals registered in the system index.</p>
 ) : (
 ...
 <span className="font-semibold text-xs text-gray-500 tracking-tight">
 {rec.block}
 </span>
 <span className={`text-[8.5px]  font-semibold px-2 py-0.5 rounded-full border ${
 qStatus.isQuarantined
 ? 'bg-red-100 border-red-200 text-red-800'
 : 'bg-emerald-100 border-emerald-200 text-green-600'
 }`}>
 {qStatus.isQuarantined ? '❌ QUARANTINED' : '✅ cleared for picking'}
 </span>
 </div>
 <p className="text-[10px] text-gray-900 font-medium font-bold ">
 Chemical: <span className="font-semibold text-gray-900 font-semibold">{rec.chemical}</span> • Target: {rec.target}
 </p>
 <div className="flex items-center gap-2 flex-wrap">
 <div className="flex items-center gap-1.5 text-xs font-semibold text-gray-900 font-medium">
 ...
 if (daysRemaining > 0) {
 return (
 <span className="text-[10px] bg-red-100 text-red-800 border border-red-200 px-2.5 py-0.5 rounded-full font-semibold font-mono animate-pulse">
 PHI Withholding: {daysRemaining} days left
 </span>
 );
 }
 return (
 <span className="text-[10px] bg-emerald-105 bg-emerald-100 text-green-600 border border-emerald-200 px-2.5 py-0.5 rounded-full font-semibold font-mono">
 Withholding Expired
 </span>
 );
 })()}
 ...
 tox = { label: 'WHO Class III: Slightly Hazardous', color: 'text-green-600 bg-emerald-50 border-emerald-200' };
 }
 return (
 <div className={`mt-2 px-3 py-1.5 border rounded-lg text-[9px] font-semibold  tracking-wide inline-block ${tox.color}`}>
 {tox.label} • PPE Required
 </div>
 );
 })()}

 <div className="flex items-center gap-4 mt-2 bg-[#ffffff]/60 p-2 rounded-xl border border-gray-100/80 max-w-sm">
 <div>
 <span className="text-[8px]  font-semibold text-gray-900 font-medium block tracking-normal leading-none">
 Interval
 </span>
 <span className="text-[11px] font-semibold text-gray-900 font-semibold block mt-0.5">
 ...
 <span className="text-[11px] font-semibold text-gray-900 block mt-0.5 font-mono">
 ...
 <div>
 <span className="text-[9px]  font-semibold text-gray-900 font-medium block tracking-normal leading-none">
 Authorized Plucking Date
 </span>
 <span className={`text-sm font-semibold font-mono block mt-1.5 ${
 qStatus.isQuarantined ? 'text-red-700' : 'text-green-600'
 }`}>
 {new Date(rec.safeDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
 </span>
 {qStatus.isQuarantined && (
 <span className="text-[9px] font-semibold text-red-600 block mt-1  italic bg-red-100/50 px-2 py-0.5 rounded-md inline-block">
 ...
 className="text-gray-900 font-medium hover:text-indigo-805 p-2 rounded transition-colors cursor-pointer m-0 border border-gray-100 hover:border-indigo-100 bg-white shadow-sm shadow-xs shrink-0"
 ...
 className="text-gray-900 font-medium hover:text-red-650 p-2 rounded transition-colors cursor-pointer m-0 border border-gray-100 hover:border-red-100 bg-white shadow-sm shadow-xs shrink-0"
 ...
 <h3 className="text-sm font-semibold  text-gray-900">Edit Spray Log</h3>
 <button onClick={() => setEditingSpray(null)} className="text-gray-900 font-medium hover:text-gray-900 font-medium font-bold m-0 cursor-pointer">✕</button>
 </div>
 <div className="space-y-3">
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Block / Unit Targeted</label>
 <input
 type="text"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Chemical Applied</label>
 <input
 type="text"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Target Pest / Disease</label>
 <input
 type="text"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">PHI (Days)</label>
 <input
 type="number"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Sprayed Date</label>
 <input
 type="date"
 ...
 <div className="grid grid-cols-2 gap-2 border-t border-dashed border-gray-200 pt-3">
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Interval Days</label>
 <input
 type="number"
 ...
 <div>
 <label className="text-[10px] font-semibold text-gray-900 font-medium  block mb-1">Next Spray Date</label>
 <input
 type="date"
