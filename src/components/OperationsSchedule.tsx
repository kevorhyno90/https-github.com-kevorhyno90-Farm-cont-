...
const getDaysDiffText = (targetDateStr: string) => {
 ...
 badgeStyle: 'bg-slate-800 text-white font-semibold border-white/15'
 };
 }
};
...
export default function OperationsSchedule({ onTriggerSectionReport }: OperationsScheduleProps = {}) {
 ...
 return (
 <div className="space-y-6 font-sans antialiased text-white animate-fadeIn" id="timetable-root">
 ...
 {/* Header Banner */}
 <div className="bg-slate-900 border border-white/10 rounded-3xl p-6 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
 ...
 <span className="text-[10px] text-white font-medium font-medium font-black tracking-widest uppercase">
 ...
 className="bg-slate-900 /10 hover:bg-slate-900 /15 text-white border border-white/20 text-[10px] font-black uppercase px-4 py-2.5 rounded-xl transition-colors shrink-0 cursor-pointer m-0"
 >
 ⚡ Immediate Test Push
 </button>
 )}
 ...
 className="bg-slate-800 hover:bg-slate-800 border border-slate-700/60 text-indigo-200 font-semibold text-[10px] px-2.5 py-1.5 rounded-lg"
 >
 🐕 Canine Vaccines (10s)
 </button>
 <button
 ...
 : 'bg-slate-900 text-white font-medium font-medium hover:text-white border-slate-205'
 }`}
 >
 ...
 <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-800 border border-white/10 p-4 rounded-2xl">
 ...
 : 'bg-slate-900 text-white font-medium font-medium hover:text-white border-white/15'
 }`}
 >
 ...
 className="bg-slate-900 border border-white/15 rounded-lg text-[10px] font-black uppercase px-2.5 py-1.5 text-white font-semibold w-full sm:w-auto"
 >
 ...
 <div className="flex items-center gap-1.5 text-white font-medium font-medium font-bold">
 ...
 <div className="bg-slate-800 p-4 rounded-3xl border border-white/10 space-y-4">
 ...
 <p className="text-[11px] text-white font-medium font-medium italic text-center py-6">No pending chores in this filter.</p>
 ) : (
 filteredItems.filter(i => i.status === 'Pending').map(item => {
 const diffText = getDaysDiffText(item.targetDate);
 return (
 <div key={item.id} className="bg-slate-900 p-4 rounded-2xl border border-slate-150 shadow-xs space-y-3 relative group">
 <div>
 <span className="text-[9px] bg-slate-800 text-slate-650 px-1.5 py-0.5 rounded font-black uppercase">{item.category}</span>
 <h4 className="text-xs font-black text-white mt-2 leading-snug">{item.operation}</h4>
 </div>
 <div className="text-[10px] text-white font-medium font-medium font-bold space-y-0.5">
 <p>🕒 {item.when}</p>
 ...
 className="py-1.5 px-2 bg-emerald-900/20 hover:bg-emerald-100 text-emerald-850 border border-emerald-200 text-[10px] font-black uppercase rounded-lg cursor-pointer font-bold"
 ...
 <p className="text-[11px] text-white font-medium font-medium italic text-center py-6">No tasks in progress.</p>
 ) : (
 filteredItems.filter(i => i.status === 'In Progress').map(item => {
 const diffText = getDaysDiffText(item.targetDate);
 return (
 <div key={item.id} className="bg-slate-900 p-4 rounded-2xl border border-slate-150 shadow-xs space-y-3 relative">
 <div>
 <span className="text-[9px] bg-slate-800 text-slate-650 px-1.5 py-0.5 rounded font-black uppercase">{item.category}</span>
 <h4 className="text-xs font-black text-white mt-2 leading-snug">{item.operation}</h4>
 </div>
 ...
 className="w-1/2 py-1.5 bg-slate-800 hover:bg-slate-800 text-white font-semibold border text-[10px] font-black uppercase rounded-lg cursor-pointer"
 >
 ← Revert
 </button>
 <button
 ...
 <p className="text-[11px] text-white font-medium font-medium italic text-center py-6">No completed tasks yet.</p>
 ) : (
 filteredItems.filter(i => i.status === 'Completed').map(item => {
 return (
 <div key={item.id} className="bg-slate-900 /80 p-4 rounded-2xl border border-white/15 shadow-xs space-y-3 opacity-90 relative">
 ...
 <div className="text-[10px] text-slate-455 text-white font-medium font-medium font-bold space-y-0.5">
 <p>👤 {item.assignedTo || 'General Team'}</p>
 </div>
 <div className="flex gap-1.5 pt-2 border-t border-white/10">
 <button
 onClick={() => handleUpdateStatusDirect(item.id, 'In Progress')}
 className="w-full py-1.5 bg-slate-800 hover:bg-slate-800 text-white font-semibold border text-[10px] font-black uppercase rounded-lg cursor-pointer"
 >
 ← Move to In Progress
 </button>
 </div>
 ...
 <div className="bg-slate-900 border border-white/10 rounded-3xl p-12 text-center text-white font-medium font-medium font-bold text-xs space-y-2">
 <AlertCircle className="mx-auto text-white font-medium" size={32} />
 <p>No operations scheduled in this category.</p>
 <p className="text-[11px] font-normal text-white font-medium font-medium">Add a custom SOP using the button above.</p>
 </div>
 ) : (
 ...
 <div 
 key={item.id} 
 className={`bg-slate-900 border rounded-3xl p-5 md:p-6 transition-all shadow-xs relative overflow-hidden ${
 item.status === 'Completed' 
 ? 'border-slate-150 bg-slate-800/50 opacity-90' 
 ...
 <span className="bg-slate-150 text-white text-[8.5px] font-black px-2 py-0.5 rounded-md uppercase tracking-wide">
 {item.category}
 </span>
 <button
 ...
 {item.status === 'Completed' ? (
 <span className="bg-emerald-55 bg-emerald-900/20 text-emerald-700 border border-emerald-200 text-[8.5px] font-black px-2 py-0.5 rounded-md uppercase tracking-wide">
 ✓ Done
 </span>
 ) : (
 timeDiffBadge && (
 ...
 className="text-white font-medium font-medium hover:text-indigo-805 p-1 hover:bg-slate-55 rounded cursor-pointer border-0 bg-transparent"
 title="Edit SOP operation values"
 >
 <PenSquare size={13} className="text-white font-medium font-medium hover:text-indigo-650" />
 </button>

 {/* DELETE OP BUTTON */}
 <button
 onClick={() => handleDeleteItem(item.id)}
 className="text-white font-medium font-medium hover:text-rose-800 font-bold p-1 cursor-pointer bg-transparent border-0"
 ...
 {/* Operation Name */}
 <h3 className={`text-base font-black text-white ${item.status === 'Completed' ? 'line-through text-slate-450 text-white font-medium font-medium' : ''}`}>
 {item.operation}
 </h3>
 ...
 <div className="space-y-1">
 <span className="text-[9px] font-black uppercase text-white font-medium font-medium tracking-wider flex items-center gap-1">
 <Calendar size={11} className="text-emerald-600" /> Target Date & Team
 </span>
 <div className="text-xs space-y-1 bg-slate-800/30 p-2.5 rounded-xl border border-slate-150/50">
 ...
 <p className="text-xs font-black text-slate-850 bg-slate-800/30 p-2.5 rounded-xl border border-slate-150/50">
 {item.when}
 </p>
 </div>
 ...
 <p className="text-xs text-slate-650 bg-slate-800/30 p-2.5 rounded-xl border border-slate-150/50 leading-relaxed font-semibold">
 {item.how}
 </p>
 </div>
 ...
 <p className="text-xs text-slate-650 bg-slate-800/30 p-2.5 rounded-xl border border-slate-150/50 leading-relaxed italic font-medium">
 "{item.why}"
 </p>
 </div>
 </div>
 ...
 : 'bg-slate-800 hover:bg-slate-800 text-white font-medium font-medium hover:text-white font-medium font-medium'
 }`}
 ...
 <div className="bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 space-y-5 animate-scaleUp border border-white/10 max-h-[90vh] overflow-y-auto">
 ...
 className="text-white font-medium font-medium hover:text-white font-medium font-medium font-bold text-base bg-slate-800 rounded-full w-7 h-7 flex items-center justify-center cursor-pointer border-0 m-0"
 >
 ✕
 </button>
 </div>
 ...
 className="w-full bg-slate-800 border border-white/15 rounded-xl px-3 py-2.5 font-bold text-xs"
 >
 ...
 className="w-full bg-slate-800 border border-slate-250 focus:border-emerald-700 rounded-xl px-3 py-2.5 font-bold text-xs"
 />
 </div>
 ...
 className="w-full bg-slate-800 border border-slate-250 focus:border-emerald-700 rounded-xl px-3 py-2 font-semibold text-xs leading-relaxed"
 />
 </div>
 ...
 type="button"
 onClick={() => setShowAddModal(false)}
 className="flex-1 bg-slate-800 hover:bg-slate-800 text-white font-medium font-medium font-black uppercase text-xs rounded-xl py-3 transition-colors cursor-pointer border-0 m-0"
 >
 Cancel
 </button>
 <button
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950 font-sans animate-fadeIn">
 <div className="bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl p-6 md:p-8 space-y-6 border border-amber-200 animate-scaleUp relative overflow-hidden">
 {/* Top caution stripe */}
 ...
 <div className="bg-slate-800 border border-slate-150 p-4 rounded-2xl space-y-2">
 <span className="text-[10px] font-bold text-white font-medium font-medium uppercase tracking-widest block">Standard Operating Procedure:</span>
 <p className="text-xs text-white font-semibold font-semibold leading-relaxed">
